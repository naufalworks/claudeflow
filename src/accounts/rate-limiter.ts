 ...
 * 
 * Per-account request tracking and throttling with rate limit header parsing,
...
/**
 * Rate Limiter for per-account request tracking and throttling
 * 
 * Features:
...
export class RateLimiter {
  ...
  private readonly backoffAttempts: Map<string, BackoffEntry> = new Map();
   ...
   */
  updateFromHeaders(headers: RateLimitHeaders, window: 'minute' | 'hour' | 'day'): void {
    const resetAt = new Date(headers.reset * 1000);
    
    let currentWindow = this.windows.get(this.accountId);
    if (!currentWindow) {
      currentWindow = this.initializeWindow();
      this.windows.set(this.accountId, currentWindow);
    }
  ...
  /**
   * Check if account is near rate limit (90% threshold)
   * 
   ...
   */
  isNearLimit(): boolean {
    const window = this.windows.get(this.accountId);
    if (!window) {
      return false;
    }
  ...
  async handleRateLimitError(retryAfter?: number): Promise<void> {
    const entry = this.backoffAttempts.get(this.accountId);
    const attempt = entry ? entry.count : 0;
    
    this.backoffAttempts.set(this.accountId, {
      count: attempt + 1,
      timestamp: new Date(),
    });
  ...
  /**
   * Reset backoff counter for this account
   * 
   * Called after successful request to reset exponential backoff.
   */
  resetBackoff(): void {
    this.backoffAttempts.delete(this.accountId);
  }
   ...
   */
  getStatus(): RateLimitWindow | null {
    const window = this.windows.get(this.accountId);
    if (!window) {
      return null;
    }
  ...
  private cleanupOldBackoffEntries(): void {
    ...
    for (const [accountId, entry] of this.backoffAttempts.entries()) {
      if (entry.timestamp.getTime() < cutoff) {
        this.backoffAttempts.delete(accountId);
      }
    }
  }
}
