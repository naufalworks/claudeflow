...
export default function AccountsPage() {
  ...
  const [addStatus, setAddStatus] = useState<"idle" | "starting" | "pending" | "complete" | "error">("idle");
      ...
      <div className="border-b border-border-subtle bg-surface">
        <div className="px-6 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="primary" size="md" icon="add" onClick={openAddAccountModal} className="min-w-[140px] text-white border border-brand-600">
              Add Account
            </Button>
          </div>
                    ...
                    >
                            ...
                            <div
                              className={cn(
                                ...
                                account.quota && account.quota.requestsPerMinute
                                  ? (account.quota.requestsPerMinuteUsed / account.quota.requestsPerMinute) * 100 > 80
                                    ? "bg-red-500"
                                    : (account.quota.requestsPerMinuteUsed / account.quota.requestsPerMinute) * 100 > 60
                                    ? "bg-yellow-500"
                                    : "bg-green-500"
                                  : "bg-gray-500"
                              )}
                              style={{
                                width: account.quota && account.quota.requestsPerMinute
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={async () => {
                              if (!confirm(`Are you sure you want to delete account ${account.id}?`)) return;
          ...
          <div className="w-full max-w-lg rounded-lg border border-border-subtle bg-surface p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-text-muted">Authorize with AWS. No email or password is stored here.</p>
              </div>
              <button onClick={closeAddAccountModal} className="rounded p-1 hover:bg-surface-3" aria-label="Close">
            ...
            {kiroLogin && (addStatus === "pending" || addStatus === "complete") && (
              <div className="space-y-4">
                ...
                <div className="rounded-lg border border-border-subtle bg-surface-2 p-4">
                  ...
                  <div className="font-mono text-3xl font-bold tracking-widest text-text-main">{kiroLogin.userCode}</div>
                </div>
            ...
            <div className="mt-6 flex justify-end gap-2">
              {addStatus === "error" && <Button variant="secondary" size="sm" onClick={openAddAccountModal}>Try Again</Button>}
              <Button variant="secondary" size="sm" onClick={closeAddAccountModal}>Close</Button>
            </div>
          </div>
        </div>
      )}
